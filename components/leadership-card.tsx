...
const BackArrow = ({ onClick }: { onClick: () => void }) => (
  <button
    onClick={onClick}
    className="flex items-end justify-end self-end rounded-full bg-transparent text-[#fff] hover:bg-transparent hover:text-white"
  >
...
const ForwardArrow = ({ onClick }: { onClick: () => void }) => (
  <button
    onClick={onClick}
    className="flex items-end justify-end self-end rounded-full border-2 border-[#fff] bg-transparent text-[#fff] hover:bg-transparent hover:text-white"
  >
    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24">
...
export function LeadershipCard({ leader, isExpanded, onToggle }: LeadershipCardProps) {
  const { image, imageAlt, name, title, bio1, bio2, bio3, bio4, credentials, description } = leader;
  ...
  return (
    <ExpandableCard
      ...
      collapsedContent={
        <>
          <CardImage src={image} alt={imageAlt} heightClass="h-80" />
          <div className="flex h-[94px] flex-1 flex-col bg-[#fff] p-6 transition-all duration-300 hover:scale-[1.01]">
            ...
            <p className="flex-1 font-normal text-[#0C0D0C] text-sm" style={{ fontFamily: "Inter" }}>
              {title}
            </p>
            <ForwardArrow onClick={onToggle} />
          </div>
        </>
      }
      expandedLeft={
        <>
          <CardImage src={image} alt={imageAlt} heightClass="h-96" />
